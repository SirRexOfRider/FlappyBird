...
#Tutorial 6 and some other improvements :)
...
#Add music
theme = 'audio/banger_soundtrack.mp3'
pygame.mixer.init()
...
#Function to reset game and variables
def reset_game():
    pipe_group.empty()
    flappy.rect.x = 100
    ...
    #Return score to global variable
    return score
...
button = Button(85 , 100, button_img)
...
while run:
    ...
    #Because Flappy is actually bigger than the pipes, it's harder to score it accurately.
    #So instead, we just have a counter that will keep increasing once you pass
    #The first pipe

    if len(pipe_group) > 0:
        if bird_group.sprites()[0].rect.left > pipe_group.sprites()[0].rect.right and game_over == False:
            score += 1
